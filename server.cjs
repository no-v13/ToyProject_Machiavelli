...
app.post(["/api/counsel", "/api/chat"], async (req, res) => {
  try {
    ...
    // Send to Google Sheets Apps Script Web App asynchronously
    const recordToGoogleSheets = async () => {
      try {
        const timestamp = new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul" });
        const payload = {
          question: question,
          answer: parsedData.fullAnswer,
          timestamp: timestamp,
          time: timestamp,
          "질문": question,
          "답변": parsedData.fullAnswer,
          "현재시간": timestamp
        };

        const responseSheets = await fetch("https://script.google.com/macros/s/AKfycbySxHuW_oqHQQLxHD37jqDEXMwf7xfCS-sBryLBCBmB2LvQNfoVAbPTHdkpxiEXEsZq/exec", {
          method: "POST",
          headers: {
        ...
        console.log("Google Sheets submission status:", responseSheets.status);
      } catch (sheetError) {
        console.error("Error submitting to Google Sheets:", sheetError);
      }
    };
    
    // Fire and forget so we don't delay the main user response
    recordToGoogleSheets();
